...
class RealJobMatcher:
    ...
    def __init__(self, cv_profile: Optional[Dict] = None):
        self._init_default()  # Start with defaults
        if cv_profile:
            self._init_with_cv_profile(cv_profile)  # Then layer on CV profile specifics
    ...
    def _init_default(self):
        """Initialize with default hardcoded keywords."""
    ...
    def _init_with_cv_profile(self, cv_profile: Dict):
        """Initialize with keywords derived from CV profile."""
        self.tech_keywords.update({skill: 3.0 for skill in cv_profile.get('technical_skills', [])})
        self.experience_keywords.update({role: 2.5 for role in cv_profile.get('specific_roles', [])})

        # Add experience level keywords
        experience_level = cv_profile.get('experience_level')
        if experience_level == 'senior':
            self.experience_keywords.update({'senior': 2.5, 'lead': 2.8, 'principal': 3.0})
        elif experience_level == 'junior':
            self.experience_keywords.update({'junior': 2.0, 'graduate': 2.0})

        # Dynamically create industry keywords
        industry_category = cv_profile.get('industry_category', 'general')
        if industry_category == 'web3_blockchain':
            self.web3_keywords.update({
                'web3': 4.0, 'blockchain': 4.0, 'defi': 4.5, 'ethereum': 3.5,
                'smart contract': 4.0, 'solidity': 4.5, 'crypto': 2.5
            })
        elif industry_category == 'tech_cloud_security':
            self.industry_keywords.update({'cloud': 2.0, 'security': 2.2, 'devops': 2.0})
        elif industry_category == 'finance_accounting':
            self.industry_keywords.update({'fintech': 2.0, 'financial': 1.8, 'trading': 2.2})
    ...
    def clean_and_tokenize(self, text: str) -> List[str]:
        ...
        # Clean words, remove stop words and short words
        cleaned_words = []
        for word in words:
            # Strip common punctuation
            cleaned_word = word.strip('.,!?;:')
            if len(cleaned_word) > 2 and cleaned_word not in self.stop_words:
        ...
        for score_key, keywords_dict in keyword_categories:
            for keyword, weight in keywords_dict.items():
                ...
                if cv_count > 0 and job_count > 0:
                    ...
                    score_contribution = weight * min(cv_count, job_count)
...
def match_score(cv_text: str, job_text: str, cv_profile: Optional[Dict] = None) -> float:
    """
    ...
    matcher = RealJobMatcher(cv_profile)
    ...
    final_score = (
            keyword_scores.get('web3_score', 0) * 0.35 +
            keyword_scores.get('tech_score', 0) * 0.35 +
            keyword_scores.get('experience_score', 0) * 0.25 +
            keyword_scores.get('industry_score', 0) * 0.05
    )
...
def analyze_match(cv_text: str, job_text: str, cv_profile: Optional[Dict] = None) -> Dict[str, any]:
    """
    ...
    keyword_scores = matcher.calculate_keyword_scores(cv_text, job_text)
    overall_score = match_score(cv_text, job_text, cv_profile)
    ...
    all_keywords = {**matcher.web3_keywords, **matcher.tech_keywords, **matcher.experience_keywords,
                    **matcher.industry_keywords}
    for keyword in all_keywords.keys():
        if keyword in job_text.lower():
            important_job_keywords.add(keyword)

    missing_important = {keyword for keyword in important_job_keywords if keyword not in cv_text.lower()}

    # Generate analysis
    analysis = {
        'overall_score': round(overall_score, 3),
        'score_breakdown': {
            'web3_relevance': round(keyword_scores.get('web3_score', 0), 3),
            'technical_skills': round(keyword_scores.get('tech_score', 0), 3),
            'experience_match': round(keyword_scores.get('experience_score', 0), 3),
            'industry_knowledge': round(keyword_scores.get('industry_score', 0), 3)
        },
        'job_level': job_level,
...
def get_application_priority(score: float, keyword_scores: Dict[str, float]) -> str:
    ...
    if score >= 0.7 and keyword_scores.get('web3_score', 0) >= 0.5:
        return 'High'
    elif score >= 0.5:
        return 'Medium'
    elif score >= 0.3:
...
def generate_real_recommendations(overall_score: float, keyword_scores: Dict[str, float],
    ...
    if job_level == 'senior' and keyword_scores.get('experience_score', 0) < 0.5:
...
def batch_match_jobs(cv_text: str, jobs_list: List[Dict], cv_profile: Optional[Dict] = None) -> List[Dict]:
    ...
    for i, job in enumerate(jobs_list):
        try:
            ...
            job['match_score'] = match_score(cv_text, job_text, cv_profile)
            job['match_analysis'] = analyze_match(cv_text, job_text, cv_profile)
    ...
    if jobs_list:
        print(
            f"🏆 Top match: {jobs_list[0]['title']} at {jobs_list[0]['company']} ({jobs_list[0]['match_score']:.3f})")
...
def test_real_matching():
    ...
    # Simulate a CV profile
    cv_profile = {
        'technical_skills': ['python', 'javascript', 'node.js', 'react', 'solidity'],
        'specific_roles': ['web3 engineer', 'developer'],
        'experience_level': 'senior',
        'industry_category': 'web3_blockchain'
    }

    score = match_score(cv_text, sample_job, cv_profile)
    analysis = analyze_match(cv_text, sample_job, cv_profile)
